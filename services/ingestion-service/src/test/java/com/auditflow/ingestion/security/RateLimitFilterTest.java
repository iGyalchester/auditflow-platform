...
class RateLimitFilterTest {
    ...
    void burstThen429WithRetryAfterPerClient() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            filter.doFilter(request("/api/v1/events", "10.0.0.1"), ok, chain);
            assertThat(ok.getStatus()).isEqualTo(200);
        }
        MockHttpServletResponse limited = new MockHttpServletResponse();
        filter.doFilter(request("/api/v1/events", "10.0.0.1"), limited, chain);
        ...
        filter.doFilter(request("/api/v1/events", "10.0.0.2"), other, chain);
        assertThat(other.getStatus()).isEqualTo(200);
    }

    @Test
    void forwardedForIsOnlyTrustedWhenConfigured() {
        MockHttpServletRequest request = request("/api/v1/events", "10.9.9.9");
    ...
    void disabledOrNonApiPathsAreNotLimited() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            disabled.doFilter(request("/api/v1/events", "10.0.0.1"), new MockHttpServletResponse(), chain);
        }
